...
export const mintOneToken = async (
  ...
  burnToken: anchor.web3.PublicKey, //fancy frenchie token
  burnAddress: anchor.web3.PublicKey, //burn address
  ...
  //acer
  const fromTokenAccount = await getTokenWallet(payer, burnToken);
    console.log('fromTokenAccount', fromTokenAccount.toString());
  const toTokenAccount = await getTokenWallet(burnAddress, burnToken);
  let tokenAccountExists = true;
  try{
    const tokenCount = await connection.getTokenAccountBalance(toTokenAccount);
  }
  catch(error){
